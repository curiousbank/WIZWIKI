...
export default class extends Controller {
  ...
  renderProfiles() {
    ...
    if (this.hasVisibleCountTarget) {
      this.visibleCountTarget.textContent = `${visibleRows.length} visible profiles`
    }
  ...
  renderTeams() {
    ...
    const visibleRows = this.filteredRows().filter((row) => this.topTraits(row).length > 0)
    ...
    teams.forEach((team, index) => {
      ...
      const strengthSpread = new Set(team.flatMap((row) => this.topTraits(row))).size
      card.innerHTML = `
        ...
        <div class="mt-2 grid gap-2">
          ${team.map((row) => `
            <div class="border border-dotted border-white/15 bg-black/60 p-2 font-mono text-xs text-zinc-200">
              <span class="text-white">${this.escapeHtml(row.dataset.name || "Unknown")}</span>
              <span class="text-zinc-500"> // ${this.escapeHtml(this.topTraits(row).join(", "))}</span>
            </div>
          `).join("")}
        </div>
      `
  ...
  buildTeams(rows, size) {
    ...
    while (pool.length > 0) {
      ...
      const usedTraits = new Set(this.topTraits(leader))
      const usedDomains = new Set(this.domains(leader))
      ...
      while (members.length < size && pool.length > 0) {
        ...
        pool.forEach((row, index) => {
          const uniqueDomains = this.domains(row).filter((domain) => !usedDomains.has(domain)).length
          const uniqueTraits = this.topTraits(row).filter((trait) => !usedTraits.has(trait)).length
        ...
        this.topTraits(candidate).forEach((trait) => usedTraits.add(trait))
        this.domains(candidate).forEach((domain) => usedDomains.add(domain))
      }
  ...
  syncDomainCountsAndButtons() {
    ...
    baseRows.forEach((row) => {
      this.domains(row).forEach((domain) => counts.set(domain, (counts.get(domain) || 0) + 1))
    })
  ...
  filteredRows() {
    return this.rowsForStatusAndDomain().filter((row) => {
      ...
      return this.topTraits(row).some((trait) => this.selectedTraits.has(trait))
    })
  }
  ...
  rowsForStatusAndDomain() {
    ...
    return this.rowsForStatus().filter((row) => this.domains(row).some((domain) => this.selectedDomains.has(domain)))
  }
  ...
  traitCounts(rows) {
    const counts = new Map()
    rows.forEach((row) => {
      this.topTraits(row).forEach((trait) => counts.set(trait, (counts.get(trait) || 0) + 1))
    })
    return counts
  }
