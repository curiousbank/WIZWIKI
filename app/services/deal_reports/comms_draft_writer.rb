...
INDUSTRY_COMPANY_KEYWORDS = [
  ...
  [/\b(restoration|water damage|fire damage|mitigation)\b/i, "Restoration"]
    ...
    def deterministic_fast_path_draft
      ...
      if support_rag_profile? && defined?(Comms::PbSmsCommands)
        support_body = Comms::PbSmsCommands.reply(inbound)
        return support_fast_path_draft(support_body) if support_body.present?
      end
    ...
    def support_fast_path_draft(body)
      body = safe_persisted_sms_body(body)
      return if support_sms_quality_rejection_reason(body).present?

      {
        "body" => body,
        "provider" => "local/pb_command",
        "model" => "deterministic_pb_sms_command",
        "writer_model" => writer_model,
        "writer_model_label" => WizwikiSettings.sms_writer_model_label(writer_model),
        ...
        "sms_quality_gate" => "fast_path",
        "draft_source" => "pb_command",
        "reason" => "A recognized 313.cash command used the deterministic low-message play path.",
        "conversation_state" => conversation_state,
        "operator_prompt" => @operator_prompt.presence,
        "background_queued" => false
      }.compact_blank
    ...
    def apply_worker_answer!(question)
      ...
      result = {
        ...
        "reason" => guardrail_override.to_h["reason"].presence || (copilot_only ? "Generated by Copilot for human approval after the first request returned." : (support_rag_profile? ? "Generated from the selected 313.cash support RAG after the first request returned." : "Generated by Alice local worker after the first request returned.")),
    ...
    def support_alice_prompt(json_output: false)
      ...
      <<~PROMPT.squish
        You are Thumper von AUTOS, the concise 313.cash and Powderball SMS support guide.
        #{output_instruction}
        Use only facts in the supplied 313.cash FAQ/guide documents and the current SMS thread. Never use unrelated sales facts, checkout links, CRM sales language, or unrelated memory.
        Answer the newest question first in plain language. Give at most one next step and stay under #{MAX_SMS_CHARS} characters.
        Commands GO, RPS, GUESS, AIRDROP, POWDERBALL, and HELP are handled deterministically outside this prompt.
        Never claim a coin transfer, airdrop, wager, or game entry happened from the text itself. Real transactions require the member to sign in and confirm through 313.cash.
        If the RAG does not support an answer, say so briefly and send the user to https://313.cash/guide or https://313.cash/faq. Do not invent account, wallet, balance, price, or transaction facts.
...
def ollama_draft
    ...
    def support_sms_quality_rejection_reason(body)
      ...
      return "support_sms_wrong_source" if text.match?(/\b(?:checkout|shopify)\b/i)
      if defined?(Comms::SmsBodySafety) && Comms::SmsBodySafety.internal_leak?(text)
        return "support_sms_internal_leak"
      end
      if text.match?(/\b(?:i|we)\s+(?:sent|transferred|claimed|entered|placed)\b.{0,45}\b(?:coin|airdrop|wager|bet|powderball|transaction)\b/i)
    ...
    def support_fallback_draft(error = nil)
      body = if latest_inbound_sms.to_s.squish.present?
        "I couldn't verify that from the 313.cash guide yet. Start here: https://313.cash/guide You can also reply GO for the secure play menu."
      else
        "Welcome to 313.cash support. Reply GO for the secure play menu, or open https://313.cash/guide to get started."
      end

      {
        "body" => body,
        "provider" => "local/support_fallback",
        "model" => "313_cash_safe_fallback",
        "writer_model" => writer_model,
        "writer_model_label" => WizwikiSettings.sms_writer_model_label(writer_model),
        "rag_profile" => rag_profile.fetch("key"),
        "rag_profile_label" => rag_profile.fetch("label"),
        ...
        "reason" => "The selected support RAG could not produce a verified answer, so the safe guide fallback was used.",
        "conversation_state" => conversation_state,
        "operator_prompt" => @operator_prompt.presence,
...
- If the latest customer message is only a 5-digit number and the prior bot question asked for ZIP, service area, location, specific area, neighborhood, route area, or mailing area, treat it as a ZIP code. Do not turn it into a home count, sign quantity, or budget.
    ...
    def support_alice_context_payload
...

      {
        ...
        support_contract: {
          source_of_truth: "313.cash /faq and /guide",
          commands: %w[GO HELP RPS GUESS AIRDROP POWDERBALL],
          play_url: "https://313.cash/play",
          transactions_require_signed_in_confirmation: true,
          unknown_answer: "Send the user to /guide or /faq; never invent a fact."
        },
        ...
        current_next_text: current_sms_body_for_context,
        worker_rule: "Return only one concise customer SMS. Use selected 313.cash support documents only. Never mix in WIZWIKI sales material or claim a transaction was executed by SMS."
      }.compact_blank
    end
    ...
    def rag_semantic_query
      ...
      [
        latest_inbound_sms,
        @operator_prompt,
        "313.cash Powderball onboarding airdrop RPS guess number account phone SMS support"
